...
impl Engine {
    ...
    pub fn ctrl(
        ...
        unsafe {
            cvt(ffi::ENGINE_ctrl(self.as_ptr(), cmd, i, p, f))?;
        }
        Ok(())
    }
    ...
    #[inline]
    pub fn ctrl_cmd(&mut self, cmd: &str, i: i64, p: *mut c_void, f: extern "C" fn(), optional: i32) -> Result<(), ErrorStack> {
        let cmd = CString::new(cmd).unwrap();
        unsafe {
            cvt(ffi::ENGINE_ctrl_cmd(self.as_ptr(), cmd.as_ptr(), i, p, f, optional))?;
        }
        Ok(())
    }
    ...
    pub fn ctrl_cmd_string(
        ...
        unsafe {
            cvt(ffi::ENGINE_ctrl_cmd_string(self.as_ptr(), cmd.as_ptr(), arg.as_ptr(), optional))?;
        }
        Ok(())
    }
    ...
    pub fn get_id(&mut self) -> Result<String, ErrorStack> {
        println!("engine ptr {:?}", self.as_ptr());
        unsafe {
...
mod test {
    ...
    // TODO lookup what error type should be returned here
    fn create_custom_engine() -> Result<Engine, ErrorStack> {
        // So, many "stars must align" to setup custom engine support in openssl.
        //
        // 1. OpenSSL must be compiled with "dynamic" engine support.
        // 2. There must be a SO present that implements the OpenSSL engine interface.
        // 3. You must setup the engine prior to using it.

        // As a user, you MUST call "ENGINE_load_builtin_engines" to ensure that the "dynamic"
        // engine is loaded and ready for your use/overriding.
        Engine::load_builtin_engines();

        let mut engine = Engine::by_id("dynamic")?;

        // TODO we need to figure out how to export the test binary as an OpenSSL engine for use
        // in these tests...

        engine.ctrl_cmd_string("SO_PATH", "THIS_EXE", 0)?;
        engine.ctrl_cmd_string("ID", "test", 0)?;
        engine.ctrl_cmd_string("LOAD", "NULL", 0)?;
        engine.ctrl_cmd_string("CMD_TEST", "hello world!", 0)?;
    ...
    #[test]
    fn custom_engine() {
        // let result = create_custom_engine();

        // if let Err(ref error_stack) = result {
